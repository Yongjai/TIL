...
하지만 이렇게 사용을 하는 것은 좋지 않습니다. 왜냐하면 NewModel.h에서도 MyModel.h의 프로퍼티를 가져다가 사용해야 하는 경우가 발생할 수도 있기 때문입니다. 헤더 파일끼리 서로 참조를 하게 되면 컴파일 자체가 되지 않습니다. 만약에 A와 B라는 헤더가 존재하고 서로 참조를 하고 있다고 가정해봅시다. 컴파일을 하게 되면 A는 B의 프로퍼티를 가져와야 하고 B도 A의 프로퍼티를 가져와야 해서 컴파일러는 A와 B를 왔다갔다 하면서 무한루프에 빠지게 됩니다. 
